(function () {
  ...
  async function loadMembers(){
    ...
    try {var resp=await fetch('/api/concept-temperature/members?window='+encodeURIComponent(windowEl.value)+'&concept_code='+encodeURIComponent(selected));var data=await resp.json();if(!resp.ok||!data.ok)throw new Error(data.error||'加载失败');
      memberTitle.textContent=data.concept.concept_name+' · 成分股';memberMeta.textContent='共 '+data.stocks.length+' 只有效成分股 · 按 '+data.window+' 日前复权涨幅降序 · 截至 '+data.as_of_date;
  ...
  windowEl.addEventListener('change',function(){selected='';memberTitle.textContent='成分股清单';memberMeta.textContent='点击左侧概念查看，按 X 日前复权涨幅降序。';membersEl.innerHTML='<tr><td colspan="5" class="empty">尚未选择概念</td></tr>';loadConcepts();});
