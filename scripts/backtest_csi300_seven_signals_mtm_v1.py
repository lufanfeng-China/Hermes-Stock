...
LOT_CASH = 50_000.0
OUTPUT = Path("/mnt/c/Users/Sky.Lu/Desktop/output/CSI300_七方案_逐起点年度MTM_v1_20260728.txt")
...
def load_constituents() -> list[str]:
    """Fetch the current official CSI300 list once and normalize six-digit codes."""
...
def prepare_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    pct5y = close.rolling(PERCENTILE_WINDOW, min_periods=PERCENTILE_WINDOW).rank(pct=True)
...
def load_data(codes: list[str]) -> dict[str, pd.DataFrame]:
    ...
    for n, code in enumerate(codes, 1):
        try:
            ...
            continue
        if frame is None or len(frame) < PERCENTILE_WINDOW + 121:
            continue
        ...
        for code, action in sorted(pending_buy.pop(date, [])):
            ...
            if action == "new":
                if code in positions:
                    continue
                positions[code] = Position(shares=shares, cost=cost)
                cash -= cost
                stats["entries"] += 1
            elif action == "add":
...
def main() -> None:
    ...
    report = []
    report.append("CSI300 七方案：逐起点、逐自然年 MTM 回测 v1")
    report.append("=" * 78)
    ...
    report.append("全局过滤（本次明确假设）：非高位=收盘价近1200交易日分位<80%；非空头=MA20>=MA60。")
    report.append("方案2使用用户指定的近1200交易日分位<40%，替代全局<80%条件；其余全局条件仍适用。")
    ...
    for scheme, name in SCHEME_NAMES.items():
        ...
        report.append("起点  首年收益  次年收益  累计收益  期末权益  原始信号/开仓/补仓/卖出/期末持仓")
    ...
    report.append("方案  2012首年  2013次年  期末权益  累计收益  开仓  补仓  卖出  期末持仓")
    for scheme, name in SCHEME_NAMES.items():
