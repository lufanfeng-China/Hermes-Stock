#!/usr/bin/env python3
"""688019 EMA(6, EMA6-18, 108) strategy, chronological strict-MTM backtest.
...
STEM = "688019_EMA6_EMA18_EMA108_严格MTM_2012起_20260731_v1"
...
def simulate(frame: pd.DataFrame) -> tuple[dict, list[dict], list[dict], list[dict]]:
    ...
    signal_counts = {"raw_entry_signals": 0, "new_entry_orders_scheduled": 0, "add_orders_scheduled": 0,
                     "sell_orders_scheduled_h3": 0, "sell_orders_scheduled_profit": 0, "cash_rejected": 0}
    ...
    for i, date in enumerate(dates):
        ...
        if position is not None:
            ...
            # "跌至H3以下且2个交易日内无法站上H3": two consecutive closes
            # at/below H3 after a close below it.  A close strictly above H3 resets.
            position.below_h3_days = position.below_h3_days + 1 if close <= float(row.h3) else 0
            exit_reason = None
            if position.below_h3_days >= 2:
                exit_reason = "close_at_or_below_h3_for_2_consecutive_trading_days"
                signal_counts["sell_orders_scheduled_h3"] += 1
            elif position.armed and (bool(row.macd_dead_cross) or pnl < 0.15):
                exit_reason = "armed_macd_dead_cross" if bool(row.macd_dead_cross) else "armed_profit_fell_below_15pct"
                signal_counts["sell_orders_scheduled_profit"] += 1
...
def report_text(payload: dict) -> str:
    s = payload["summary"]
    lines = [
        "688019（澜起科技）EMA(6)-EMA(EMA6,18)-EMA(108) 策略：严格 MTM 回测 v1",
        "=" * 88,
        ...
        "H3退出：收盘<=H3连续2个交易日（即两天均未严格站上H3），下一交易日开盘全卖。",
        "盈利退出：总浮盈达到>=20%后武装；随后MACD(12,26,9)死叉，或总浮盈严格跌破15%，下一交易日开盘全卖。",
...
def main() -> None:
    ...
    payload = {
        "version": "v1", "code": CODE, "name": "澜起科技", "summary": summary,
        "contracts": {"signal": "H1 crosses above H2; H1>H3; H2>H3; H1/H3-1<10%; H3>REF(H3,1)",
                      ...
                      "h3_exit": "close <= H3 for two consecutive trading days -> next open sell",
                      "profit_exit": "arm at total close PnL >=20%; then MACD dead cross or total close PnL <15% -> next open sell",
