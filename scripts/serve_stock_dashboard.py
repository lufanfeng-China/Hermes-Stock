...
def _generate_concept_narrative(
    ...
    # Strip suffix for keyword matching
    ...
    # Extract keywords from concept name for matching
...
def _calc_industry_multiplier(concept_name: str, industry_display: str) -> float:
    ...
    if not industry_display:
        return 0.4  # unknown industry, penalize
    concept_kw = concept_name.replace("概念", "").replace("板块", "").replace("产业", "").strip()
...
def _extract_concept_keywords(concept_name: str) -> list[str]:
    ...
    import re
    # Try to split Chinese compound words
    # For "机器人概念" → ["机器人"]
    # For "固态电池" → ["固态电池", "电池", "固态"]
    # For "低空经济" → ["低空经济", "低空", "经济"]
    keywords = [stripped]
    if len(stripped) >= 3:
        keywords.append(stripped[:2])  # first 2 chars
    if len(stripped) >= 4:
        keywords.append(stripped[2:])  # last chars
    if len(stripped) >= 5:
        keywords.append(stripped[1:3])  # middle 2 chars
...
class StockDashboardHandler(BaseHTTPRequestHandler):
    ...
    def handle_concept_analysis(self, query: str) -> None:
        """Search concept and return enriched stock list."""
        params = {k: v[0] for k, v in parse_qs(query).items() if v}
        q = str(params.get("q", "")).strip()
        if not q:
            self.respond_json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": "q required"})
            return
        try:
            from app.search.index import search_concept_stocks, load_rps_rows, load_industry_valuation_rows, _coerce_float, _coerce_int, _load_financial_snapshot
            result = search_concept_stocks(q)
            if not result.get("concept"):
                self.respond_json(HTTPStatus.OK, {"ok": True, "concept": None, "stocks": [], "method": result.get("method")})
                return
            ...
            for m in members:
                ...
                for vrow in valuation_rows:
                    for mv in (vrow.get("member_valuation_rows") or []):
                        if str(mv.get("market","")).strip() == market and str(mv.get("symbol","")).strip() == symbol:
                            pe = _coerce_float(mv.get("pe_ttm"))
                            break

                total_rps = sum(_coerce_float(rps.get(f"rps_{w}")) or 0 for w in (20, 50, 120, 250))

                # Multi-factor match percentage:
                # Factor A: concept prominence (rank within stock's concepts) — weight 20%
                # Factor B: industry alignment (does business match concept?) — weight 70%
                # Factor C: related concept density (other matching tags) — weight 10%
                ...
                concept_list = m.get("concept_list", [])
                ...
                if rank is not None and isinstance(rank, int) and rank >= 1 and total and total >= rank:
                    ...
                    prominence = 40  # moderate default for unknown
                ...
                # Weighted final: 20% prominence + 70% industry + 10% density
                match_pct = round(prominence * 0.2 + industry_score * 0.7 + density_score * 0.1)
                match_pct = min(100, max(0, match_pct))

                # Generate narrative
                narrative = _generate_concept_narrative(concept_name, industry, concept_list, rank, total)
            ...
            # Sort by match_pct descending (None → end)
