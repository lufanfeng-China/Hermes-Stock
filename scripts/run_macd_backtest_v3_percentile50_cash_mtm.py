#!/usr/bin/env python3
"""Versioned MACD extreme-GC backtest: entry signal requires trailing 5Y close percentile <50%."""
...
STATE_FILE = PROJECT_ROOT / "data/derived/datasets/final/macd_gc_state_percentile50_v3.json"
MTM_FILE = PROJECT_ROOT / "data/derived/datasets/final/macd_gc_equity_weekly_percentile50_v3.json"
CONSTITUENT_FILES = (
...
def build_signal_bars(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pct5y = close.rolling(1200, min_periods=240).rank(pct=True)
    buy = golden & (bars["ndif"] < -1.0) & (ma10 > ma10.shift(1)) & (pct5y < 0.50)
    bars["buy_signal"] = buy.fillna(False)
    bars["signal_pct5y"] = (pct5y * 100).round(2)
