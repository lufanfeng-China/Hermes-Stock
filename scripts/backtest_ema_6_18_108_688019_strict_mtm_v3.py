#!/usr/bin/env python3
"""688019 EMA(6, EMA6-18, 108) strategy v3, chronological strict-MTM backtest.
...
STEM = "688019_EMA6_EMA18_EMA108_无止损_H3盈利跌破卖出_H3乖离40pct止盈_严格MTM_2012起_20260731_v3"
...
def simulate(frame: pd.DataFrame) -> tuple[dict, list[dict], list[dict], list[dict]]:
    ...
    signal_counts = {"raw_entry_signals": 0, "new_entry_orders_scheduled": 0, "add_orders_scheduled": 0,
                     "sell_orders_scheduled_h3_break_nonloss": 0, "sell_orders_scheduled_h3_distance_gt_40pct": 0,
                     "cash_rejected": 0}
    ...
    for i, date in enumerate(dates):
        ...
        if position is not None:
            ...
            # There is no stop-loss: a H3 break exits only if the total position
            # has a non-negative close PnL.  The 40% trigger is close/H3-1 > 40%.
            pnl = close * position.shares / position.cost - 1.0
            exit_reason = None
            if close / float(row.h3) - 1.0 > 0.40:
                exit_reason = "close_h3_distance_strictly_gt_40pct"
                signal_counts["sell_orders_scheduled_h3_distance_gt_40pct"] += 1
            elif close < float(row.h3) and pnl >= 0.0:
                exit_reason = "close_broke_below_h3_with_nonnegative_position_pnl"
                signal_counts["sell_orders_scheduled_h3_break_nonloss"] += 1
...
def report_text(payload: dict) -> str:
    s = payload["summary"]
    lines = [
        "688019（澜起科技）EMA(6)-EMA(EMA6,18)-EMA(108) 策略：无止损/H3盈利跌破卖出/H3乖离40%止盈，严格 MTM 回测 v3",
        "=" * 88,
        ...
        "不设止损：收盘价跌破H3但持仓总收盘盈亏为负时，继续持有、不卖出。",
        "卖出：收盘价相对H3的乖离严格大于40%〔close/H3-1>40%〕，或收盘价严格跌破H3且持仓总收盘盈亏>=0；均下一可用交易日开盘全卖。",
...
def main() -> None:
    ...
    payload = {
        "version": "v3", "code": CODE, "name": "澜起科技", "summary": summary,
        "contracts": {"signal": "H1 crosses above H2; H1>H3; H2>H3; H1/H3-1<10%; H3>REF(H3,1)",
                      ...
                      "h3_break_nonloss_exit": "close < H3 and total close PnL >= 0 -> next open sell; negative PnL does not stop out",
